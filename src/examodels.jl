...
# The linked list with `inner` represents a sum of expressions `∑_{i in data} head(i)`
...
#exagen(i::IteratorIndex) = ExaModels.DataIndexed(ExaModels.DataSource(), i.value)
...
function _exafy_con(
    ...
    bin = update_bin!(
        bin,
        ExaModels.DataIndexed(ExaModels.DataSource(), 2) =>
            0 * ExaModels.Var(1) + e,
        (c, con_to_idx[i]),
    )
...
function _exafy(e::MOI.ScalarQuadraticTerm{T}, var_to_idx, p = ()) where {T}
    if e.variable_1 == e.variable_2
        ...
        return ExaModels.DataIndexed(ExaModels.DataSource(), length(p) + 1) *
               abs2(v),
...
MOI.is_empty(model::ExaOptimizer) = isnothing(model.model)
...
function MOI.optimize!(optimizer::ExaOptimizer)
...
function MOI.get(model::ExaOptimizer, attr::MOI.ObjectiveValue)
    MOI.check_result_index_bounds(model, attr)
