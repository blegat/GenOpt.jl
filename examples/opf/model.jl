...
function build_model(pm::Dict)
    ...
    bus_pd = Dict(
        k => sum(ref[:load][l]["pd"] for l in ref[:bus_loads][k]; init = 0.0) for (k, v) in ref[:bus]
    )
    bus_qd = Dict(
        k => sum(ref[:load][l]["qd"] for l in ref[:bus_loads][k]; init = 0.0) for (k, v) in ref[:bus]
    )
    bus_gs = Dict(
        k => sum(ref[:shunt][s]["gs"] for s in ref[:bus_shunts][k]; init = 0.0) for (k, v) in ref[:bus]
    )
    bus_bs = Dict(
        k => sum(ref[:shunt][s]["bs"] for s in ref[:bus_shunts][k]; init = 0.0) for (k, v) in ref[:bus]
    )
    ...
    @variable(
        model,
        ref[:gen][i]["qmin"] <=
        qg[i in keys(ref[:gen])] <=
        ref[:gen][i]["qmax"]
    )
    @variable(
        model,
        -ref[:branch][ref[:arcs][i][1]]["rate_a"] <=
        p[i in 1:narc] <=
        ref[:branch][ref[:arcs][i][1]]["rate_a"]
    )
    @variable(
    ...
    @objective(
        ...
        Min,
        lazy_sum(
            cost1[i] * pg[i]^2 + cost2[i] * pg[i] + cost3[i] for
            i in keys(ref[:gen])
        ),
    )
